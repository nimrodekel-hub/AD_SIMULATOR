...
| 5 | `generate-scenario.ts` | דילמה + רמת קושי + **פרופיל המערכת** | **מערך יירוט לריצה** — מסלולים, מהירויות וזמני הופעה |
| 6 | `generate-gui.ts` | צילומי מסך + **פרופיל המערכת** + בקשות השינוי של המתכנן | שלד HTML של הקונסולה |
| 7 | `debrief.ts` | **יומן הריצה** + הספירה + הידע | ציון + טקסט תחקיר |
...
לעבוד עליה. אחר כך רצף של ארבעה שלבים, לכל מערכת בנפרד. **הסדר אינו המלצה:**
...
sequenceDiagram
    ...
    Note over D: 3ב. חזרה גנרלית — מטרות על הקונסולה, בלי חניך ובלי לכתוב כלום

    Note over D: 4. לימוד דילמות
    D->>V: ...ראה סעיף 6
```
...
### חזרה גנרלית — מטרות על הקונסולה לפני שהיא מאושרת

התצוגה המקדימה בבנייה היא שלד ריק. היא עונה אם הפאנלים במקום ולא יותר מזה: היא
לא יכולה לומר אם רשימת הטרקים מחזיקה את השורות שלה, אם המכ"ם נשאר מרובע כשמצוירת
בו תמונה, ואם בקרות הירי שורדות פתרון ירי שמופיע לידן. את זה רואים רק כשדברים
זזים.

`/designer/systems/<id>/gui/rehearse` מריץ את **אותו** `LiveRun`, אותו מנוע ואותו
...
בשלב מסוים. **בלי קריאה למודל, בלי דילמה, ובלי לכתוב שום דבר** — מתכנן שבודק
פריסה לא צריך לחכות שבעים שניות, לשלם, או שתהיה כבר דילמה מאושרת. זה עובד על
טיוטה כמו על קונסולה מאושרת, כי לבדוק לפני שמאשרים זה כל הרעיון.

למה לא לכתוב "תצוגה מקדימה" נפרדת: **חזרה גנרלית מול העתק־כמעט של הדבר האמיתי
לא מוכיחה כלום על הדבר האמיתי.**
...
לכן הדילמות המאושרות של המערכת מוצגות כרשימה, ובחירה מתוכה פותחת הרצה ישירות.
ההרצה נרשמת עם `requested_text` שאומר `Chose "…" from the list.` — כי הרצה בלי
שום סיבה רשומה גורמת לתדריך ולהיסטוריה אצל המדריך להיראות כאילו אף אחד לא ביקש
כלום. **המצב עצמו עדיין נבנה מחדש בכל פעם**, כך שאותה דילמה פעמיים אינה אותה
הרצה פעמיים.
...
sequenceDiagram
    ...
    D->>V: POST /api/systems/<id>/dilemmas/extract
    V->>G: commit — dilemma-job.json, status: running
    V-->>D: 202 מיד — הדפדפן לא מחזיק חיבור
    V->>A: תמלול מלא + structured output
    A-->>V: רשומה מובנית
    V->>G: commit — dilemma-job.json, status: done
...
sequenceDiagram
    ...
    V->>G: commit — scenario-<trainee>.json, status: running
    ...
    V-->>T: הפניה לקונסולה, כשהתרחיש מוכן
...
**סירוב הוא אירוע, לא שגיאה.** ירי מתחת לטווח המזערי, או כששתי רקטות כבר
באוויר, הוא בדיוק הטעות שהתרגיל קיים כדי לחשוף. המנוע אומר לא **ואומר למה**,
הסירוב נכנס ליומן, והתחקיר מדבר עליו.
...
└── lib/
    ...
    │   └── rehearsal.ts     ← תרחיש בקוד, לחזרה הגנרלית של המתכנן
    └── store/
